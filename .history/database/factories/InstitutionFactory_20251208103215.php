<?php

namespace Database\Factories;

use Illuminate\Database\Eloquent\Factories\Factory;

class InstitutionFactory extends Factory
{
    // القائمة الكاملة لمؤسسات التعليم العالي في غزة (17 مؤسسة)
    private static array $institutions = [
        // الجامعات (6)
        'الجامعة الإسلامية بغزة',
        'جامعة الأزهر - غزة',
        'جامعة الأقصى',
        'جامعة القدس المفتوحة - فروع غزة',
        'جامعة غزة',
        'جامعة فلسطين',
        'جامعة الإسراء', // (أحياناً تصنف حديثاً كجامعة)

        // الكليات الجامعية (تمنح بكالوريوس ودبلوم)
        'الكلية الجامعية للعلوم التطبيقية',
        'الكلية الجامعية للعلوم والتكنولوجيا - خانيونس',
        'كلية فلسطين التقنية - دير البلح',
        'كلية العودة الجامعية',
        'كلية تنمية القدرات الجامعية',
        'كلية الصحابة الجامعية',
        'كلية الزيتونة الجامعية',
        'كلية الرباط الجامعية',
        'كلية نماء للعلوم والتكنولوجيا',

        // كليات المجتمع المتوسطة
        'كلية مجتمع الأقصى للدراسات المتوسطة',
    ];

    public function definition(): array
    {
        return [
            // إذا كان العمود في قاعدة البيانات JSON (يدعم عربي وإنجليزي) استخدم هذا الشكل:
            /*
            'name' => [
                'ar' => $this->faker->randomElement(self::$institutions),
                'en' => 'University Name English Placeholder', // ستحتاج لقائمة إنجليزية مقابلة ليكون دقيقاً
            ],
            */

            // أما إذا كان العمود String عادي (كما في الكود الذي أرسلته)، استخدم هذا:
            'name' => $this->faker->randomElement(self::$institutions),
        ];
    }
}