...
class Application extends Model
{
    ...
    /**
     * Get enabled training types based on settings
     */
    public static function getEnabledTrainingTypes(): array
    {
        ...
        if ($settings->enable_training_type_university) {
            $types[] = [
                'id' => self::TRAINING_TYPE_UNIVERSITY,
                'name' => self::TRAINING_TYPES[self::TRAINING_TYPE_UNIVERSITY]
            ];
        }

        if ($settings->enable_training_type_practice) {
            $types[] = [
                'id' => self::TRAINING_TYPE_PRACTICE,
                'name' => self::TRAINING_TYPES[self::TRAINING_TYPE_PRACTICE]
            ];
        }
    ...
    /**
     * Check if a trainee is eligible to apply for a specific training type
     * Returns [status => 'eligible'|'blocked'|'reapply', message => string, trainee => ?Trainee]
     */
    public static function checkEligibility(string $nationalId, int $trainingType): array
    {
        ...
        $settings = app(\App\Settings\TrainingSettings::class);
        $canReapply = ($trainingType === self::TRAINING_TYPE_UNIVERSITY)
            ? $settings->can_university_reapply
            : $settings->can_practice_reapply;

        $query = self::where('trainee_id', $trainee->id)
            ->where('training_type', $trainingType)
            ->whereNull('deleted_at');

        if ($canReapply) {
            // Only block if there's an application NOT in Ended status
            $blockingApplication = (clone $query)->where('status', '!=', self::STATUS_ENDED_TRAINING)->first();

            if ($blockingApplication) {
                return [
                    'status' => 'blocked',
                    'message' => self::getStatusMessage($blockingApplication->status, $trainingType),
                    'trainee' => $trainee
                ];
            }

            // Check if they have a finished application to trigger re-apply mode
            $hasFinished = $query->where('status', self::STATUS_ENDED_TRAINING)->exists();
            if ($hasFinished) {
                return [
                    'status' => 'reapply',
                    'message' => '',
            ...
            // Block if ANY application exists
