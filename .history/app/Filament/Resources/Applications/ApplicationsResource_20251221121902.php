...
class ApplicationsResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        if ($user->isSectionHead()) {
            // Assuming relationship 'section' exists on User: hasOne(Sections::class, 'hos')
        ...
        if ($user->isDepartmentHead()) {
            // Assuming relationship 'department' exists on User: hasOne(Departments::class, 'hod')
        ...
        if ($user->isMedicalManager()) {
            // User linked to Administrative via 'medical_hoa'
            // Must belong to that Administrative AND be a Medical Department
        ...
        if ($user->isAdministrative()) {
            // User linked to Administrative via 'hoa'
            return $query->whereHas('section', function ($q) use ($user) {
                $q->where('administrative_id', $user->administrative?->id);
            });
        }

        // 7. Ministry (MOH): View professional practice applications
        if ($user->isMinistry()) {
            // TRAINING_TYPE_PROFESSIONAL = 2
            return $query->where('training_type', \App\Helpers\Constans::TRAINING_TYPE_PROFESSIONAL);
        }
