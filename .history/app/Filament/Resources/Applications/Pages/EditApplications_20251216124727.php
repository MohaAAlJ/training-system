...
class EditApplications extends EditRecord
{
    ...
    {
        // If current user is a college user, enforce their college/institution on the application
        $user = Auth::user();
        if ($user instanceof \App\Models\User && $user->is()) {
        ...
        // If trainee fields were edited, persist them to the related trainee record.
        if (!empty($data['trainee_id'])) {
            DB::beginTransaction();
            try {
                $trainee = Trainees::find($data['trainee_id']);
                if ($trainee) {
                    // assemble dob from parts if provided
