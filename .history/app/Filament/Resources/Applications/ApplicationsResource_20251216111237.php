    ...
    public static function getEloquentQuery(): Builder
{
    ...
    // 4. المدير الإداري (التعديل هنا)
    if ($user->isAdministrative()) {
        
        // أولاً وقبل كل شيء: حصر المدير داخل "إدارته" فقط
        // (هذا السطر يطبق على الطبي والغير طبي والـ IT وكل المدراء)
        $query->whereHas('department', function ($q) use ($user) {
            $q->where('administrative_id', $user->administrative?->id);
        });

        // ثانياً: الفلترة الداخلية حسب نوع المدير
        
        // إذا كان "مدير طبي" -> نحصره زيادة ونخليه يشوف "الأقسام الطبية" فقط داخل إدارته
        ...
        // أما إذا كان "غير طبي" (مثل مدير الـ IT):
        // لن نضيف أي كود هنا.
        // النتيجة: سيرى كل الأقسام (الطبية وغير الطبية) ولكن *محصورين داخل إدارته* بسبب الشرط "أولاً".
