                ...
                Fieldset::make('البيانات الشخصية للمتدرب')
                    ->schema([
                        ...
                        Select::make('institution_id')
                            ...
                            ->options(fn() => Institution::where('is_active', true)->pluck('name', 'id')->toArray())
                        ...
                        Select::make('college_id')
                            ...
                            ->options(function (callable $get) {
                                if (Auth::user()->isCollegeSupervisor()) {
                                    $query = College::where('id', Auth::user()->college?->id);

                                    // Optionally check if the supervisor's college itself is active? 
                                    // The requirement says "if the college is inactive hide this college... from the external application form selecting".
                                    // Since this block is for the supervisor themselves, maybe they should see it, but let's stick to the "selectable" logic.
                                    // But actually, if they are the supervisor, it's their dashboard. Blocking them might be wrong if they need to see data.
                                    // However, the prompt says "hide ... from the application form select".
                                    // This form is used for Creating/Editing applications.
                                    // If the college is inactive, maybe they shouldn't create new applications?
                                    // Let's filter active for now, safest bet based on "hide".

                                    return $query->where('is_active', true)->pluck('name', 'id')->toArray();
                                }

                                $institutionId = $get('institution_id');
                                if ($institutionId) {
                                    // Check if Institution is active is implicitly handled because they can't select an inactive institution 
                                    // (unless it's an edit of an old record, but we are filtering options here).
                                    // Requirement: "if the institution is inactive hide it and its all colleges"

                                    // Requirement: "if the college is inactive hide this college"
