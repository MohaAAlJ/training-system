                ...
                Fieldset::make('تفاصيل الطلب')
                    ->schema([
                        Select::make('training_type')
                            ->label('نوع التدريب')
                            ->options([
                                'cooperative' => 'تدريب جامعي', // Cooperative Training
                                'professional' => 'مزاولة مهنة', // Professional Practice
                            ])
                            // 1. تحديد القيمة الافتراضية بناءً على الدور
                            ->default(function () {
                                $user = Auth::user();

                                if ($user->isCollegeSupervisor()) {
                                    return 'cooperative'; // للكليات دائماً تدريب جامعي
                                }

                                if ($user->isMinistry()) {
                                    return 'professional'; // للوزارة دائماً مزاولة مهنة
                                }

                                return null; // للأدمن يتركه فارغ ليختار هو
                            })
                            // 2. إغلاق الحقل لغير الأدمن (يظهر رمادي ولا يمكن تعديله)
                            ->disabled(fn() => ! auth()->user()->isAdmin())
                            // 3. ضروري جداً لحفظ القيمة حتى لو كان الحقل مغلقاً
                            ->dehydrated()
                            ->required(),
