                                ...
                                Select::make('major_id')
                                    ...
                                    ->options(function (callable $get) {
                                        ...
                                        if ($user instanceof \App\Models\User && $user->isCollege()) {
                                            ...
                                            return \App\Models\Major::whereHas('colleges', function ($q) use ($collegeId) { $q->where('colleges.id', $collegeId); })->get()->mapWithKeys(fn($m) => [$m->id => $m->getTranslation('name', 'ar')])->toArray();
                                        }

                                        return $get('college_id') ? \App\Models\Major::whereHas('colleges', function ($q) use ($get) { $q->where('colleges.id', $get('college_id')); })->get()->mapWithKeys(fn($m) => [$m->id => $m->getTranslation('name', 'ar')])->toArray() : [];
                                    })
                                    ->searchable()
                                    ->required()
