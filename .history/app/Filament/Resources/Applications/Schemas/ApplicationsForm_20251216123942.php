                ...
                Fieldset::make('البيانات الشخصية للمتدرب')
                    ->schema([
                        TextInput::make('national_id')
                            ->label('رقم الهوية')
                            ->formatStateUsing(fn($record) => $record?->trainee?->national_id) // جلب البيانات من المتدرب
                            ->disabled() // منع التعديل لأن البيانات في جدول آخر
                            ->dehydrated(false), // عدم محاولة الحفظ في جدول الطلبات
                        ...
                        // بالنسبة لتاريخ الميلاد، يفضل عرضه كحقل واحد للعرض بدلاً من 3 حقول
                        TextInput::make('dob')
                            ->label('تاريخ الميلاد')
                            ->formatStateUsing(fn($record) => $record?->trainee?->dob?->format('Y-m-d'))
                            ->disabled()
                            ->dehydrated(false),
