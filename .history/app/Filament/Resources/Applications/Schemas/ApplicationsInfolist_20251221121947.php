                ...
                Section::make('تفاصيل التدريب')
                    ...
                    ->schema([
                        ...
                        TextEntry::make('training_type')
                            ->label('نوع التدريب')
                            ->formatStateUsing(fn(?int $state): string => \App\Helpers\Constans::TRAINING_TYPE_LABELS[$state] ?? '-'),
                        TextEntry::make('status')
                            ->label('الحالة')
                            ->badge()
                            ->color(fn($state) => \App\Helpers\Constans::STATUS_COLORS[$state] ?? 'gray')
                            ->formatStateUsing(fn($state) => \App\Helpers\Constans::STATUS_LABELS[$state] ?? $state),
