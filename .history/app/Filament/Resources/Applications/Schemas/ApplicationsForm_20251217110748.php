                    ...
                    ->schema([
                        ...
                        Select::make('college_id')
                            ...
                            ->options(function (callable $get) {
                                // إذا تم اختيار مؤسسة، نجلب كلياتها
                                $institutionId = $get('institution_id');
                                if ($institutionId) {
                                    return \App\Models\College::where('institution_id', $institutionId)->pluck('name', 'id');
                                }
                                return [];
                            })
                            // التعبئة التلقائية لمشرف الكلية
                            ->default(function () {
                                if (()->isCollegeSupervisor()) {
                                    return auth()->user()->college_id;
                                }
                                return null;
                            })
                            // قفل الحقل لمشرف الكلية
                            ->disabled(fn() => auth()->user()->isCollegeSupervisor())
                            // ضمان إرسال القيمة
                            ->dehydrated()
                            ->required()
