...
use Filament\Actions\Action;use Filament\Schemas\Components\Fieldset;
...
class ApplicationsForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->components([
                // حقل مخفي لحفظ ID المتدرب (ضروري لصفحة الإنشاء)
                Hidden::make('trainee_id'),

                Fieldset::make('البيانات الشخصية للمتدرب')
                    ->schema([
                        // 1. حقل الاسم الكامل (مع زر التعديل الذكي)
                        TextInput::make('full_name')
                            ->label('الاسم الكامل')
                            // عرض القيمة من العلاقة
                            ->formatStateUsing(fn($record) => $record?->trainee?->full_name)
                            // في حالة التعديل: مغلق + لا يرسل بياناته للسيرفر
                            ->disabled(fn($context) => $context === 'edit')
                            ->dehydrated(fn($context) => $context === 'create') 
                            ->required()
                            // زر التعديل (يظهر فقط في صفحة التعديل وللأدمن أو المشرف)
                            ->suffixAction(
                                Action::make('edit_trainee_details')
                                    ...
                                    // تعبئة النافذة بالبيانات الحالية
                                    ...
                                    // تنفيذ التحديث عند الضغط على حفظ في النافذة
                        ...
                        // 2. باقي الحقول (تظهر فقط للقراءة في التعديل، وللإدخال في الإنشاء)
                        ...
                        // حقول المؤسسة والكلية (المنطق السابق كما هو)
                        ...
                        Select::make('college_id')
                            ...
                            ->options(function (callable $get) {
                                ...
                                if ($institutionId) return College::where('institution_id', $institutionId)->pluck('name', 'id');
                                return [];
                            })
                        ...
                        Select::make('major_id')
                            ...
                            ->disabled(fn($context) => $context === 'edit') // التعديل يتم عبر الزر
                            ->dehydrated(fn($context) => $context === 'create')
                            ->searchable()
                            ->required()
                            ->preload(),
                ...
                Fieldset::make('تفاصيل الطلب')
                    ->schema([
                        // ... (نفس كودك السابق لبيانات الطلب دون تغيير) ...
