...
class ApplicationsResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        // 2. مشرف الكلية: يرى طلاب كليته فقط
        ...
        // 4. المدير الإداري (هنا الحل الجذري)
        if ($user->isAdministrative()) {

            // الخطوة الأولى (الإجبارية للجميع - طبي أو غير طبي):
            // لا تعرض أي شيء خارج "رقم المديرية" الخاصة بهذا المستخدم
            // هذا السطر هو الذي سيخفي الصيدلة عنك لأن رقم مديريتها (6) يختلف عن مديريتك (7)
            return $query->whereHas('department', function ($q) use ($user) {
                $q->where('administrative_id', $user->administrative?->id);
            })->when($user->administrative?->is_medical === true, function ($q) {
                // الخطوة الثانية (فلترة إضافية للمدير الطبي فقط):
                // إذا كان "مدير طبي"، نحصره أكثر ليرى فقط الأقسام "الطبية" داخل إدارته
                return $q->whereHas('department', function ($subQ) {
                    $subQ->where('is_medical', true);
                });
            });
        }
