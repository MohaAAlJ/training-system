...
class ApplicationsResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        // 4. المدير الإداري (هنا كان الخطأ وتم إصلاحه)
        if ($user->isAdministrative()) {
            
            // الخطوة الأولى (الأساسية):
            // إجبار المدير (سواء طبي أو IT) أن يرى فقط البيانات التابعة لـ "رقم إدارته"
            // هذا السطر هو الذي سيخفي الصيدلة عنك لأنها تتبع إدارة أخرى
            $query->whereHas('department', function ($q) use ($user) {
                $q->where('administrative_id', $user->administrative?->id);
            });

            // الخطوة الثانية (التخصيص للمدير الطبي فقط):
            // إذا كان مدير طبي، نحصره بزيادة داخل الأقسام الطبية
            // أما إذا كان IT (غير طبي) سيتجاوز هذا الشرط ويرى كل أقسامه التي جلبناها في الخطوة الأولى
