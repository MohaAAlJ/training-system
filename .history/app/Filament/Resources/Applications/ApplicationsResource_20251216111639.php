...
class ApplicationsResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        // 3. رئيس القسم: يرى قسمه فقط
        if ($user->isDepartmentHead()) {
            return $query->where('department_id', $user->department?->id);
        }

        // 4. المدير الإداري (التعديل الجذري هنا)
        if ($user->isAdministrative()) {

            // الخطوة الأولى (الإجبارية للجميع):
            // احصر النتائج لتكون فقط تابعة للإدارة التي يديرها هذا المستخدم
            $query->whereHas('department', function ($q) use ($user) {
                $q->where('administrative_id', $user->administrative?->id);
            });

            // الخطوة الثانية (التخصيص للمدير الطبي):
            // إذا كان المدير "طبياً"، نحصره أكثر ليرى فقط الأقسام "الطبية" داخل إدارته
            ...
            // ملاحظة: مدير الـ IT (الغير طبي) سيمر من الخطوة الأولى فقط.
            // النتيجة: سيرى كل الأقسام التابعة للـ IT، ولن يرى الصيدلة لأنها تتبع administrative_id مختلف.
