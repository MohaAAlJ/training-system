    ...
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                Section::make('التحكّم في المؤسسات والكليات (Institutions & Colleges)')
                    ->description('تحديد المؤسسات والكليات المسموح لها بإضافة طلبات جديدة. (Checked = Active)')
                    ->icon('heroicon-o-academic-cap')
                    ->schema([
                        Grid::make(1)
                            ->schema([
                                \Filament\Forms\Components\Repeater::make('active_institutions')
                                    ->label('المؤسسات المفعّلة')
                                    ->schema([
                                    ...
                                    ->addActionLabel('إضافة مؤسسة')
                                    ->dehydrated(true)
                                    ->grid(2),

                                \Filament\Forms\Components\Repeater::make('active_colleges')
                                    ->label('الكليات المفعّلة')
                                    ->schema([
                                        \Filament\Forms\Components\Select::make('college_id')
                                            ->label('الكلية')
                                            ->options(\App\Models\College::with('institution')->get()->mapWithKeys(function ($college) {
                                                return [$college->id => "{$college->institution->name} - {$college->name}"];
                                            }))
                                            ->searchable()
                                            ->required()
                                            ->disableOptionsWhenSelectedInSiblingRepeaterItems(),
                                    ])
                                    ->addActionLabel('إضافة كلية')
                                    ->dehydrated(true)
                                    ->grid(2),
                            ]),
                    ]),
