...
class CheckMaintenanceMode
{
    public function handle(Request $request, Closure $next)
    {
        ...
        // 4. Skip if already on the maintenance page or logout routes (prevent redirect loop)
        if ($request->is('Home/maintenance') || $request->is('Home/logout') || $request->routeIs('filament.*.auth.logout')) {
            return $next($request);
        }
