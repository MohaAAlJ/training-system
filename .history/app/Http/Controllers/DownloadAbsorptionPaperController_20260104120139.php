...
class DownloadAbsorptionPaperController extends Controller
{
    public function download(Request $request, Application $application)
    {
        ...
        if ($request->input('mode') === 'stream') {
            return $pdf->stream('absorption_paper_' . $application->trainee->national_id . '_' . date('Y-m-d') . '.pdf');
        }

        return $pdf->download('absorption_paper_' . $application->trainee->national_id . '_' . date('Y-m-d') . '.pdf');
    }
}
